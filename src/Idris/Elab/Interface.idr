...
mkIfaceData {vars} ifc def_vis env constraints n conName ps dets meths
    = let opts = if isNil dets
          ...
          con = MkImpTy vfc EmptyFC conName !(bindTypeNames ifc [] (pNames ++ map fst meths ++ cast vars) conty)
          bound = pNames ++ map fst meths ++ cast vars in
...
getMethDecl {vars} env nest params mnames (c, nm, ty)
    = do let paramNames = map fst params
         ty_imp <- bindTypeNames EmptyFC [] (paramNames ++ mnames ++ cast vars) ty
...
getMethToplevel {vars} env vis iname cname constraints allmeths params sig
    = do let paramNames = map fst params
         ...
         let ty_constr =
             substNames (cast vars) (map applyCon allmeths) sig.type
         ty_imp <- bindTypeNames EmptyFC [] (cast vars) (bindPs params $ bindIFace vfc ity ty_constr)
...
getConstraintHint {vars} fc env vis iname cname constraints meths params (cn, con)
    = do let ity = apply (IVar fc iname) (map (IVar fc) params)
         ...
         ty_imp <- bindTypeNames fc [] (meths ++ cast vars) fty
  ...
  where
    ...
    elabMethods conName meth_names meth_sigs
        = do -- Methods have same visibility as data declaration
             fnsm <- traverse (getMethToplevel env (collapseDefault def_vis) iname conName
                                               (map fst constraints)
                                               meth_names
                                               params) meth_sigs
             let fns = concat fnsm
    ...
    elabDefault tydecls (dfc, opts, n, cs)
        = do -- orig <- branch
             ...
             let dty = bindPs params      -- bind parameters
                     ...
                     $ substNames (cast vars) methNameMap dty

             dty_imp <- bindTypeNames dfc [] (map name tydecls ++ cast vars) dty
...
--              put Ctxt orig
    ...
    elabConstraintHints conName meth_names
        = do let nconstraints = nameCons 0 constraints
             chints <- traverse (getConstraintHint vfc env (collapseDefault def_vis) iname conName
