...
mutual
  ...
  export
  TTC ImpTy where
    toBuf (MkImpTy fc nameFC n ty)
        = do toBuf fc; toBuf nameFC; toBuf n; toBuf ty
    fromBuf
        = do fc <- fromBuf; nameFC <- fromBuf; n <- fromBuf; ty <- fromBuf
  ...
  export
  TTC ImpDecl where
    toBuf (IClaim fc c vis xs d)
        = do tag 0; toBuf fc; toBuf c; toBuf vis; toBuf xs; toBuf d
    ...
    toBuf (IDirective _ _) = throw (Corrupt "Can't write directive")
    toBuf (IPragma _ _ f) = throw (Corrupt "Can't write Pragma")
    toBuf (ILog n)
        = do tag 8; toBuf n
    ...
    fromBuf
        = case !getTag of
               0 => do fc <- fromBuf; c <- fromBuf
                       vis <- fromBuf;
                       xs <- fromBuf; d <- fromBuf
