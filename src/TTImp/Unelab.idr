...
used idx (Meta _ _ _ args) = any (used idx) (map snd args)
...
unelabBinder umode nest fc env x (Pi _ rig p ty) sctm sc scty
    = do (ty', _) <- unelabTy umode nest env ty
         ...
         pure (IPi fc rig p' nm ty' sc, VType fc (MN "top" 0))
  where
    ...
    = do defs <- get Ctxt
         Just ty <- lookupTyExact n (gamma defs)
             | Nothing => case umode of
                               ImplicitHoles => pure (Implicit fc True, VErased fc Placeholder)
                               _ => pure (IVar fc (MkKindedName (Just nt) n n), VErased fc Placeholder)
         fn <- getFullName n
         n' <- case umode of
                    NoSugar _ => pure fn
                    _ => aliasName fn
...
unelabTy' umode nest env (Meta fc n i args)
    = do defs <- get Ctxt
         ...
         Just ty <- lookupTyExact (Resolved i) (gamma defs)
             | Nothing => case umode of
                               ImplicitHoles => pure (Implicit fc True, VErased fc Placeholder)
                               _ => pure (IHole fc mkn, VErased fc Placeholder)
              ...
              NoSugar True =>
                 let x' = uniqueLocal vars x in
                     unelabBinder umode nest fc env x' b
                                  ...
                                  (renameVars (CompatExt CompatPre) !(quote env' scty))
...
unelabTy' umode nest env (TDelayed fc r tm)
    = do (tm', ty) <- unelabTy' umode nest env tm
         defs <- get Ctxt
         pure (IDelayed fc r tm', VErased fc Placeholder)
unelabTy' umode nest env (TDelay fc r _ tm)
    = do (tm', ty) <- unelabTy' umode nest env tm
         defs <- get Ctxt
         pure (IDelay fc tm', VErased fc Placeholder)
unelabTy' umode nest env (TForce fc r tm)
    = do (tm', ty) <- unelabTy' umode nest env tm
         defs <- get Ctxt
         pure (IForce fc tm', VErased fc Placeholder)
unelabTy' umode nest env (PrimVal fc c) = pure (IPrimVal fc c, VErased fc Placeholder)
unelabTy' umode nest env (PrimOp fc fn args)
    = -- If we ever see this in output, we've overevaluated
      pure (Implicit fc True, VErased fc Placeholder)
unelabTy' umode nest env (Erased fc (Dotted t))
    = unelabTy' umode nest env t
unelabTy' umode nest env (Erased fc _) = pure (Implicit fc True, VErased fc Placeholder)
unelabTy' umode nest env (TType fc _) = pure (IType fc, VType fc (MN "top" 0))
unelabTy' umode nest env (Unmatched fc _) = pure (Implicit fc True, VErased fc Placeholder)
