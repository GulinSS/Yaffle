...
findFields defs con
    = case !(lookupTyExact con (gamma defs)) of
           Just t => pure (Just !(getExpNames !(expand !(nf [<] t))))
           _ => pure Nothing
  where
    ...
    getExpNames (VBind fc x (Pi _ _ p ty) sc)
        = do rest <- getExpNames !(expand !(sc (pure (VErased fc Placeholder))))
             let imp = case p of
                            Explicit => Nothing
                            _ => Just x
             pure $ (nameRoot x, imp, getRecordType !(expand ty)) :: rest
    getExpNames _ = pure []
...
checkUpdate rig elabinfo nest env fc upds rec expected
    = do recty <- case expected of
         ...
         let solvemode = case elabMode elabinfo of
                              InLHS c => inLHS
                              _ => inTerm
         delayOnFailure fc rig elabinfo env (Just recty) needType RecordUpdate $
           \delayed =>
